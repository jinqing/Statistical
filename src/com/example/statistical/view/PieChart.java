...
public class PieChart extends View {
    ...
    int[] mSweep = { 80, 40, 20, 40, 60, 20, 60, 40 };
    int large=0;
    ...
    private void initView() {
        ...
        str=new String[mSweep.length];
        for(int i=0;i<mSweep.length;i++) {
        	large=large+mSweep[i];
        }
        for(int i=0;i<mSweep.length;i++){
        	str[i]=(decimalFormat.format((float)mSweep[i]/large*100))+"%";
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaints = new Paint[mSweep.length];
        ...
        for (int i = 0; i < mSweep.length; i++) {
            ...
            measureText(start + 180, mSweep[i], cicleWidth / 3, i);
    ...
    class mAnimation extends Animation {
        ...
        protected void applyTransformation(float interpolatedTime,
            ...
            if (interpolatedTime < 1.0f && flag == 2) {
                for (int i = 0; i < mSweep.length; i++) {
                    mSweep[i] = (int) (mSweep[i] * interpolatedTime);
                }
            } else if (flag == 1) {
                for (int i = 0; i < mSweep.length; i++) {
                    mSweep[i] = 0;
